...
- `update` now classifies a TUF trust-root **refresh** failure (network/timeout) as a retryable network condition (wrapped as a signature-download failure) instead of the non-retryable `E_INTEGRITY` reserved for a genuine signature/identity/checksum mismatch.
