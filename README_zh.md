...
| ⏱️ **分时数据** | 当日全部分钟级成交明细 |
| 🔍 **名称搜索** | 中文、拼音、英文均可搜索 |
| 🤖 **AI 友好** | 默认 JSON、`--format`/`--compact`/`--fields`、`reference`/`context`/`doctor` 自描述 |
| ⚡ **单文件** | 下载即用，无运行时依赖 |
...
| `--fields` | 仅输出指定的顶层字段（按顺序，逗号分隔） |
| `--quiet` | 抑制非结果 stdout 输出 |
| `--json` | `--format json` 的兼容别名（已废弃） |
| `--version` | 打印版本 |
| `--help` | 打印帮助 |

默认输出 JSON（稳定、低 token、可解析）。结果走 stdout，错误与进度走 stderr。人类可读表格用 `--format text`。
